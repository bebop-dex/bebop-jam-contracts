    ...
    function transferTokens(
        ...
        for (uint i; i < tokens.length; ++i) {
            if (tokenTransferTypes[i] == Commands.SIMPLE_TRANSFER) {
                ...
                IERC721(tokens[i]).safeTransferFrom(from, receiver, nftIds[curNFTsInd++]);
            } else if (tokenTransferTypes[i] == Commands.NFT_ERC1155_TRANSFER) {
                IERC1155(tokens[i]).safeTransferFrom(from, receiver, nftIds[curNFTsInd++], amounts[i], "");
            } else {
        ...
        require(curNFTsInd == nftIds.length, "INVALID_NFT_IDS_LENGTH");
